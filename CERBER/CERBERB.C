...
int GetUser_Id(void);     /* Do sprawdzenia stanu zainstalowania */
char far* LLpointer();    /* Pobiera adres struktury LL */
unsigned realDOSversion();/* Pobiera rzeczywista wersje DOS-u */
int freeenviroment();     /* Zwalnia blok srodowiska */
int Cerber21H(void);      /* Funkcja modyfikujaca serwis systemowy */
void interrupt DOShandler();/* Handler przerwania 21H */
void interrupt handler();   /* Handler przerwania zegarowego */
...
MySS=_SS;
MySP=_SP;
if(GetUser_Id()!=0xFFFF)/* Sprawdzenie zainstalowania */
	 return 2;      /* Jest juz zainstalowany !!! */
...
ERROR:
asm pop ds;
error=_AX;  /* error code 3,5,16 */
return 0xffff;
}

unsigned Del(void far* p)
{
asm push ds;
_AH=0x41;
_DX=FP_OFF(p);
...
ERROR:
asm pop ds;
error=_AX;  /* error code 2,3,5 */
return 0xffff;
}

unsigned Ren(void far* p,void far* s)
{
asm push es;
asm push di;
asm push ds;
...
ERROR:
asm pop ds; /* Restore DATA segment */
error=_AX;  /* error code 2,3,5 i 17 */
asm pop di;
asm pop es;
return 0xffff;
}
...
switch(hi(r.r_ax)){
case 0xff:if(lo(r.r_ax)==0)             	/*FF00H - Get User_ID 	*/
	    {
	    r.r_ax=0x0f0f;r.r_cx=User_Id;
	     break;				/* returned in CX    	*/
						/*and 0F0F in AX for 	*/
						/* detected TSR */
	    }
	  if(lo(r.r_ax)==1)                     /* FF01H - Settings User_Id 	*/
	    {
	    if(r.r_cx==HiPass|| r.r_dx==LoPass)
	  ...
	  if(lo(r.r_ax)==2)			/* FF02H - Set file owner */
	     {
...
case 0x3A: /* usuniecie  podkatalogu */
	{
	struct dir_entry far* this=NULL;

	if( (this=pass_to_entry(MK_FP(r.r_ds,r.r_dx)))!=NULL)/* Odszukanie pozycji wg nazwy */
		{
		if(RmDir(MK_FP(r.r_ds,r.r_dx))==0xffff)
				{
				r.r_ax=error;
				r.r_flags|=CARRY;
				goto END;
...
case 0x41: /* skasowanie   pliku */
	{
	struct dir_entry far* this=NULL;

	if( (this=pass_to_entry(MK_FP(r.r_ds,r.r_dx)))!=NULL)/* Odszukanie pozycji wg nazwy */
		{
		if(Del(MK_FP(r.r_ds,r.r_dx))==0xffff)
				{
				r.r_ax=error;
				r.r_flags|=CARRY;
				goto END;
...
case 0x56: /* zmiana nazwy   pliku */
	{
	struct dir_entry far* this=NULL;

	if(Ren(MK_FP(r.r_ds,r.r_dx),MK_FP(r.r_es,r.r_di))==0xffff)
				{
				r.r_ax=error;
				r.r_flags|=CARRY;
				goto END;
